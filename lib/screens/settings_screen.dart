...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _loadSettings() async {
    ...
    setState(() {
      ...
      desiredLayer = settings['desiredLayer'];
      useH2 = settings['useH2'];
      useH3 = settings['useH3'];
      compareAllMethods = settings['compareAllMethods'];
  ...
  void _initToleranceControllers() {
    // Очищаем старые контроллеры если есть
    for (final controllers in _toleranceControllers.values) {
      for (final controller in controllers) {
        controller.dispose();
      }
    }
    _toleranceControllers.clear();
    
    // Создаём новые контроллеры
  ...
  Widget build(BuildContext context) {
    if (_isLoading) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Параметры расчёта'),
        ),
      ...
      child: Row(
        children: [
          ...
          Expanded(
            flex: 3,
            child: TextFormField(
              controller: controllers[0], // Используем контроллер вместо initialValue
          ...
          Expanded(
            flex: 3,
            child: TextFormField(
              controller: controllers[1], // Используем контроллер вместо initialValue
              keyboardType: TextInputType.number,
              decoration: const InputDecoration(
                isDense: true,
                contentPadding: EdgeInsets.symmetric(horizontal: 8, vertical: 8),
